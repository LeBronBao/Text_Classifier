...
class ScaledDotProductAttention:
	def __init__(self, d_model, attn_dropout=0.1):
		self.temper = np.sqrt(d_model)   # 计算attention时QK点积分母上的d
		self.dropout = Dropout(attn_dropout)
	def __call__(self, q, k, v, mask):
		attn = Lambda(lambda x:K.batch_dot(x[0],x[1],axes=[2,2])/self.temper)([q, k])  # 计算QK点积并除以d的平方根
		if mask is not None:  # 进行掩码操作
			mmask = Lambda(lambda x:(-1e+10)*(1-x))(mask)
			attn = Add()([attn, mmask])
		attn = Activation('softmax')(attn)
		attn = self.dropout(attn)
		output = Lambda(lambda x:K.batch_dot(x[0], x[1]))([attn, v])  # 最终attention的结果
		return output, attn
...
class MultiHeadAttention:
	...
	def __init__(self, n_head, d_model, d_k, d_v, dropout, mode=0, use_norm=True):
		...
		self.attention = ScaledDotProductAttention(d_model)  # 初始化点积
		self.layer_norm = LayerNormalization() if use_norm else None  # 初始化Norm层
	...
	def __call__(self, q, k, v, mask=None):
		...
		if not self.layer_norm:  # 是否进行Norm处理
			return outputs, attn
		outputs = Add()([outputs, q])  # 先Add再进行Norm处理
		return self.layer_norm(outputs), attn


# 在Multi-head Attention输出后的前馈网络层
...
class EncoderLayer():
	...
	def __call__(self, enc_input, mask=None):
		# Multi-head Attention的三个输入都是一致的
...
class Encoder():
	def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, \
		...
		self.emb_layer = word_emb  # 词嵌入层
		self.pos_layer = pos_emb  # 位置嵌入层
		self.emb_dropout = Dropout(dropout)
		self.layers = [EncoderLayer(d_model, d_inner_hid, n_head, d_k, d_v, dropout) for _ in range(layers)]

	def __call__(self, src_seq, src_pos=None, return_att=False, active_layers=999):
		x = self.emb_layer(src_seq)  # 进行单词到词嵌入的转换
		if src_pos is not None:  # 进行位置嵌入的转换
			pos = self.pos_layer(src_pos)
			x = Add()([x, pos])  # 两种嵌入进行相加
		x = self.emb_dropout(x)
		if return_att: atts = []
		mask = Lambda(lambda x:GetPadMask(x, x))(src_seq)
		for enc_layer in self.layers[:active_layers]:
			x, att = enc_layer(x, mask)  # 进行编码
...
class MyTransformerEncoder:
	def __init__(self, vocab_size, len_limit, kw_len_limit, d_model=300, d_inner_hid=256, n_head=3,
				 d_k=32, d_v=32, layers=2, dropout=0.1, word_embed_matrix=None):
		self.vocab_size = vocab_size
		self.len_limit = len_limit
		self.kw_len_limit = kw_len_limit
		self.d_model = d_model
		w_embed_dim = d_model
		#pos_embed_matrix = GetPosEncodingMatrix(len_limit, w_embed_dim)

		if word_embed_matrix is not None:
			word_embed_layer = Embedding(vocab_size, w_embed_dim, trainable=False, input_length=len_limit, weights=[word_embed_matrix])
			self.word_embed_layer = word_embed_layer
			kw_word_embed_layer = Embedding(vocab_size, w_embed_dim, trainable=False, input_length=kw_len_limit, weights=[word_embed_matrix])
			self.kw_word_embed_layer = kw_word_embed_layer
		else:
			word_embed_layer = Embedding(vocab_size, w_embed_dim, )
			self.word_embed_layer = word_embed_layer

		#pos_embed_layer = Embedding(len_limit, w_embed_dim, trainable=False, input_length=len_limit, weights=[pos_embed_matrix])

		self.encoder = Encoder(d_model, d_inner_hid, n_head, d_k, d_v, layers, dropout, word_emb=word_embed_layer,)
		self.model = None

	def get_pos_seq(self, x):
		mask = K.cast(K.not_equal(x, 0), 'int32')  # 比较x和0是否相等，并转化为int32
		pos = K.cumsum(K.ones_like(x, 'int32'), 1)  # 生成一个全为1的向量，并累加
		return pos * mask

	def compile(self, optimizer='adam', active_layers=999):
		input = Input(shape=(self.len_limit,), dtype='int32')
		#kw_input = Input(shape=(self.kw_len_limit,), dtype='int32')
		#input_pos = Lambda(self.get_pos_seq)(input)

		# transformer
		encoder_output = self.encoder(input, active_layers=active_layers)
		pool = GlobalMaxPool1D()(encoder_output)

		'''
		embed = self.word_embed_layer(input)
		embed_dp = Dropout(0.3)(embed)
		kw_embed = self.kw_word_embed_layer(kw_input)
		kw_embed_dp = Dropout(0.3)(kw_embed)

		lstm = Bidirectional(LSTM(self.len_limit))(embed_dp)
		...
		kw_lstm = Bidirectional(LSTM(self.kw_len_limit))(kw_embed_dp)
		kw_lstm_dp = Dropout(0.3)(kw_lstm)

		merged_vector = concatenate([pool, lstm_dp, conv_bn, kw_lstm_dp], axis=-1)
		'''

		dense = Dense(300, activation='relu', name='denselayer')(pool)
		d_pool = Dropout(0.3)(dense)
		d_bn = BatchNormalization()(d_pool)
		pred = Dense(5, activation='softmax')(d_bn)

		self.model = Model(inputs=input, outputs=pred)
		self.model.compile(loss='sparse_categorical_crossentropy', optimizer=optimizer, metrics=['accuracy'])

	def fit_model(self, train_x, train_y, train_kw_x=None):
		if train_kw_x is None:
			self.model.fit(train_x, train_y, batch_size=64, epochs=30, verbose=2)
		else:
			self.model.fit([train_x, train_kw_x], train_y, batch_size=64, epochs=30, verbose=2)

	def evaluate(self, test_x, test_y, test_kw_x=None):
		if test_kw_x is None:
			score = self.model.evaluate(test_x, test_y, verbose=2)
			print("Accuracy:"+str(score))
		else:
			score = self.model.evaluate([test_x, test_kw_x], test_y, verbose=2)
			self.model.save_weights('trained_model/amazon_mobile/all_realtrans_w.h5')
			print("Accuracy:"+str(score))
